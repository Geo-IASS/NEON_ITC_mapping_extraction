...
sp.corr <- function(X,Y, pwd)
{
  ...
  legend("top",legend=c("Mean","95% CI"),col=c("black","red"),lwd=2, main="")
...
pls.cal <- function(train.data, comps, scaling = FALSE){
...
#--------------------------------------------------------------------------------------------------#
opt.comps <- function(pls.mod.train, train.data, plots.ok = FALSE, use.press = TRUE, h0 = FALSE)
{
  if(!h0){
    ncomps = rep(0,5)
  }else{
  ...
  for (j in 1:5) {
    ...
    if(!use.press){
      rms <- RMSEP(eval(parse(text = paste('pls.mod.train$',names(Y)[j],sep=""))), newdata = train.PLS)$val
      if(plots.ok){    
        plot(RMSEP(eval(parse(text = paste('pls$',names(Y)[j],sep=""))),estimate=c("test"),newdata = train.PLS), main="MODEL RMSEP",
             xlab="NUM OF COMPONENTS",ylab="Model Validation RMSEP",lty=1,col="black",cex=1.5,lwd=2)
        
        r2 <- R2(eval(parse(text = paste('pls.mod.train$',names(Y)[j],sep=""))), newdata = train.PLS)
        plot(R2(eval(parse(text = paste('pls.mod.train$',names(Y)[j],sep=""))),estimate=c("test"),newdata = train.PLS), main="MODEL R2",
             xlab="NUM OF COMPONENTS",ylab="Model Validation RMSEP",lty=1,col="black",cex=1.5,lwd=2)
      }
    }else{
      #--------------------------------------------------------------------------------------------------#
      # Calculate Q2 statistic
      q2<- Q2(train.PLS, eval(parse(text = paste('pls.mod.train$',names(Y)[j],sep="")))) 
    }
    if(!h0){
      if(!use.press){
      ...
      ncomps <- pmin(0.8 * length(train.data[,1]), ncomps)
    }else{
      if(!use.press){
        ncomps[[names(train.data[,2:6])[j]]]  <- rms[seq(2,length(rms),2)]
      }else{
        ncomps[[names(train.data[,2:6])[j]]] <- q2
      }
    }
  }
  return(ncomps)
...
res.out <- function(pred.val.data, t.data)
{
  out <- list()
  traits <- as.matrix(t.data[,2:6])
  for (j in 1:5) {
    # Build output dataset
    # PLSR Summary statistics
    pred.data <- as.vector(eval(parse(text = paste('pred.val.data$',names(t.data[,2:6])[j],sep=""))))
...
step.fwd <- function(test.data,train.data){
  ...
  for (jk in 1:5) {
    ...
    data.test <- data.frame(test.traits[,jk], test.spec)
    colnames(data.test)[1] <- "Y"
    reg.best=regsubsets(Y~.,data=data , nvmax =best,  method='forward')
    tr.MSE = sum((traits[,jk] - predict.regsubsets(reg.best, as.data.frame(data) ,id=as.integer(best)))^2) / length(traits[,jk])
    tr.R2 = 1 - length(traits[,jk]) / sum((traits[,jk]- mean(traits[,jk]))^2) * tr.MSE
    
    best.pred = predict.regsubsets(reg.best, data.test, id=as.integer(best))
    stp.mse <- mean((best.pred - test.traits[,jk])^2)
    stp.testR2 <- 1-length(test.traits[,1]) * stp.mse / sum((test.traits[,jk]-mean(test.traits[,jk]))^2)
    out[[names(train.data[,2:6])[jk]]] <- list(mod= reg.best, coeff=coef(reg.best, best), mse= stp.mse,  stp.testR2 = stp.testR2, npar=best, tr.MSE= tr.MSE, tr.R2=tr.R2)
  }
  return(out)
}
...
lasso.asd <- function(test.data,train.data)
{
  ...
  for (j in 1:5) {
    ...
    out[[names(train.data[,2:6])[j]]] <- list(mod= lasso.mod, coeff = lasso.coef, mse = lasso.mse, r2=lasso.r2, lambda = bestlam, tr.R2 = tr.R2, tr.MSE = tr.MSE)
  }
  return(out)
}
